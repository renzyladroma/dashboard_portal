...
class Daily_Report extends CI_Controller {
	...
	public function get_report_location_table(){
		...
		$query = $this->db->query("SELECT advertiser_report_per_hour.date AS ads_date, FORMAT(SUM(total_impression),0) AS total_impression, FORMAT(SUM(total_click),0) AS total_click
...
WHERE advertiser_report_per_hour.date BETWEEN '$start_date' AND '$end_date'");
		...
		if($query->num_rows() > 0){
			...
			echo "<tfoot>
							<tr>
								<th>Total</th>
								<th>$row_total->total_impression</th>
								<th>$row_total->total_click</th>
							</tr>
						</tfoot>
					</table>";
		}else{
...
GROUP BY advertiser_report_per_hour.date ORDER BY advertiser_report_per_hour.date ASC");
		...
		$total = $this->db->query("SELECT advertiser_report_per_hour.date AS ads_date, FORMAT(SUM(total_impression),0) AS total_impression, FORMAT(SUM(total_click),0) AS total_click
FROM advertiser_report_per_hour
WHERE advertiser_report_per_hour.date >= DATE(NOW()) - INTERVAL 7 DAY");
